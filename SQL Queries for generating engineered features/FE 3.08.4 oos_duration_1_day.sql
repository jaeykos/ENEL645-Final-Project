CREATE OR REPLACE TABLE `just-data-sandbox-oos.feature_engineering.FE3084` AS
WITH time_ranges AS (
  SELECT
    unique_id,
    order_item_name,
...
overlapping_oos AS (
  SELECT
    ...
    GREATEST(
      TIMESTAMP_SUB(c.event_time, INTERVAL 24 HOUR),
      p.event_time
    ) AS overlap_start,  
    LEAST(
      c.event_time,
    ...
    CASE 
      WHEN p.scenario = 'OUT_OF_STOCK' AND TIMESTAMP_DIFF(
        ...
        GREATEST(TIMESTAMP_SUB(c.event_time, INTERVAL 24 HOUR), p.event_time),
  ...
  JOIN time_ranges AS p
   ...
   AND p.event_time >= TIMESTAMP_SUB(c.event_time, INTERVAL 24 HOUR)
)
SELECT 
  t.unique_id,
  t.order_item_name,  
  ...
  COALESCE(SUM(o.overlapping_minutes), 0) AS oos_duration_last_24hr_minutes  -- Sum overlapping minutes, default to 0 if no OOS
