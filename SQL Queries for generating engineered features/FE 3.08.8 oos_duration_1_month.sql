CREATE OR REPLACE TABLE `just-data-sandbox-oos.feature_engineering.FE3088` AS
WITH time_ranges AS (
  SELECT
    unique_id,
    order_item_name,
    ...
    scenario,
    LEAD(createdTime) OVER (PARTITION BY order_item_name ORDER BY createdTime) AS next_event_time
...
overlapping_oos AS (
  SELECT
    ...
    GREATEST(
      TIMESTAMP_ADD(c.event_time, INTERVAL -30 DAY),  -- Using TIMESTAMP_ADD with -30 DAYS instead of TIMESTAMP_SUB
      p.event_time
    ) AS overlap_start,  
    LEAST(
      c.event_time,
    ...
    CASE 
      WHEN p.scenario = 'OUT_OF_STOCK' AND TIMESTAMP_DIFF(
        ...
        GREATEST(TIMESTAMP_ADD(c.event_time, INTERVAL -30 DAY), p.event_time),
  ...
  JOIN time_ranges AS p
    ON c.order_item_name = p.order_item_name
   AND p.event_time < c.event_time
   AND p.event_time >= TIMESTAMP_ADD(c.event_time, INTERVAL -30 DAY)
)
SELECT 
  t.unique_id,
  t.order_item_name,  
  ...
  COALESCE(SUM(o.overlapping_minutes), 0) AS oos_duration_last_1month_minutes  -- Sum overlapping minutes, default to 0 if no OOS
FROM time_ranges AS t
LEFT JOIN overlapping_oos AS o
  ON t.order_item_name = o.order_item_name
 AND t.event_time = o.createdTime
GROUP BY t.unique_id, t.order_item_name, t.event_time, t.scenario
ORDER BY t.order_item_name, t.event_time DESC;
