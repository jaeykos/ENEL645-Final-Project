CREATE OR REPLACE TABLE `just-data-sandbox-oos.feature_engineering.FE312` AS
WITH profit_data AS (
    SELECT
        unique_id,
        order_item_name,
...
aggregated_profit AS (
    ...
    ON p.order_item_name = d.order_item_name
    AND d.createdTime < p.createdTime

    GROUP BY p.unique_id, p.order_item_name, p.createdTime, p.scenario
)
