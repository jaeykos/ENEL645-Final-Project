CREATE OR REPLACE TABLE `just-data-sandbox-oos.feature_engineering.FE3086` AS
WITH time_ranges AS (
  SELECT
    unique_id,
    order_item_name,
...
SELECT 
  ...
  COALESCE(SUM(o.overlapping_minutes), 0) AS oos_duration_last_3_days  -- Sum overlapping minutes, default to 0 if no OOS
