CREATE OR REPLACE TABLE `just-data-sandbox-oos.feature_engineering.FE1055` AS
WITH time_ranges AS (
  SELECT
    unique_id,
    restaurant_id,
...
overlapping_oos AS (
  SELECT
    ...
    GREATEST(
      TIMESTAMP_ADD(c.event_time, INTERVAL -3 DAY),  -- Use INTERVAL -3 DAYS
      p.event_time
    ) AS overlap_start,  
    LEAST(
      c.event_time,
    ...
    CASE 
      WHEN p.scenario = 'OUT_OF_STOCK' AND TIMESTAMP_DIFF(
        ...
        GREATEST(TIMESTAMP_ADD(c.event_time, INTERVAL -3 DAY), p.event_time),
  ...
  JOIN time_ranges AS p
   ...
   AND p.event_time >= TIMESTAMP_ADD(c.event_time, INTERVAL -3 DAY)
)
SELECT 
  t.unique_id,
  t.restaurant_id,   
  ...
  COALESCE(SUM(o.overlapping_minutes), 0) AS oos_duration_last_3DAY_minutes  -- Sum overlapping minutes, default to 0 if no OOS
