CREATE OR REPLACE TABLE `just-data-sandbox-oos.feature_engineering.FE1052` AS
WITH time_ranges AS (
  SELECT
    unique_id,
    restaurant_id,
...
SELECT 
  ...
  COALESCE(SUM(o.overlapping_minutes), 0) AS oos_duration_last_3hr_minutes  -- Sum overlapping minutes, default to 0 if no OOS
