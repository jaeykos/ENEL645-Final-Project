CREATE OR REPLACE TABLE `just-data-sandbox-oos.feature_engineering.FE3081` AS
WITH time_ranges AS (
  SELECT
    unique_id,
    order_item_name,
    createdTime AS event_time,  -- Explicit alias for clarity
    scenario,
    LEAD(createdTime) OVER (PARTITION BY order_item_name ORDER BY createdTime) AS next_event_time
  FROM `just-data-sandbox-oos.feature_engineering.cleaned_data`  -- Include all rows
),
overlapping_oos AS (
  SELECT
    c.unique_id,
    c.order_item_name,
    c.event_time AS createdTime,  -- Rename it back to match original column name
    c.scenario AS current_scenario,  
    p.event_time AS prev_time,  
    p.scenario AS prev_scenario,  
    p.next_event_time AS prev_next_event_time,  
    GREATEST(
      TIMESTAMP_ADD(c.event_time, INTERVAL -1 HOUR),  -- Use INTERVAL -1 HOUR
      p.event_time
    ) AS overlap_start,  
    LEAST(
      c.event_time,
      COALESCE(p.next_event_time, c.event_time)
    ) AS overlap_end,  
    CASE 
      WHEN p.scenario = 'OUT_OF_STOCK' AND TIMESTAMP_DIFF(
        LEAST(c.event_time, COALESCE(p.next_event_time, c.event_time)),
        GREATEST(TIMESTAMP_ADD(c.event_time, INTERVAL -1 HOUR), p.event_time),
        MINUTE  -- Calculate difference in minutes
      ) > 0 THEN 
        TIMESTAMP_DIFF(
          LEAST(c.event_time, COALESCE(p.next_event_time, c.event_time)),
          GREATEST(TIMESTAMP_ADD(c.event_time, INTERVAL -1 HOUR), p.event_time),
          MINUTE  -- Calculate difference in minutes
        )
      ELSE 0 
    END AS overlapping_minutes  -- Rename to reflect minutes
  FROM time_ranges AS c
  JOIN time_ranges AS p
    ON c.order_item_name = p.order_item_name
   AND p.event_time < c.event_time
   AND p.event_time >= TIMESTAMP_ADD(c.event_time, INTERVAL -1 HOUR)
)
SELECT 
  t.unique_id,
  t.order_item_name,  
  t.event_time AS createdTime,  -- Ensure createdTime is explicitly included
  t.scenario AS current_scenario,  
  COALESCE(SUM(o.overlapping_minutes), 0) AS oos_duration_last_1hr_minutes  -- Sum overlapping minutes, default to 0 if no OOS
FROM time_ranges AS t
LEFT JOIN overlapping_oos AS o
  ON t.order_item_name = o.order_item_name
 AND t.event_time = o.createdTime
GROUP BY unique_id, t.order_item_name, t.event_time, t.scenario
ORDER BY t.order_item_name, t.event_time DESC;
