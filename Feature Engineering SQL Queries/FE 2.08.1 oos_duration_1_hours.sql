CREATE OR REPLACE TABLE `just-data-sandbox-oos.ENEL_645.FE2081` AS
WITH time_ranges AS (
  SELECT
    unique_id,
    restaurant_id,
  ...
  FROM `just-data-sandbox-oos.ENEL_645.cleaned_data`  -- Include all rows
),
overlapping_oos AS (
  SELECT
    c.unique_id,
...
GROUP BY unique_id, t.restaurant_id, t.order_item_name, t.event_time, t.scenario
